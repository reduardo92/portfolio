...
window.addEventListener("scroll", ()=>{
    if (window.scrollY > navBar.offsetHeight + 300) navBar.classList.add("scrollNav");
    else navBar.classList.remove("scrollNav");
});
...
const animateCursor = (e)=>{
    ...
    if (item.contains("logo--img") || item.contains("nav--links") || item.contains("icon") || item.contains("btn")) cursor.classList.add("cursor-animate");
    else cursor.classList.remove("cursor-animate");
};
...
const joinWords = (target)=>{
    ...
    const splitText = ele.textContent.split("").map((i)=>i === " " ? ` <span class='span--title'>${i}</span>` : `<span class='span--title'>${i}</span>`).join("");
...
const animateHome = ()=>{
    ...
    // SUBTITLE ,BTN,LINKS
    tl.fromTo([
        heroContent.children[2],
        heroContent.children[3],
        heroContent.children[4]
    ], {
        scale: 0.9,
        opacity: 0
    }, {
...
function addLinkClass(id, target, tl) {
    id === target ? tl.to(`.nav--container a[href='#${target}']`, {
...
},{"../scss/main.scss":"4Pg3x"}],"4Pg3x":[function() {},{}]},["8UT0b","8lRBv"], "8lRBv", "parcelRequire05e8")
